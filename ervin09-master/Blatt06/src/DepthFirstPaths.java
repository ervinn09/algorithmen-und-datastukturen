...
public class DepthFirstPaths {
    ...
    public void distto(Graph t){
        for (int vertex = 0; vertex< t.V (); vertex++) {
            if (!hasPathTo (vertex)){    //wenn keine Path existiert für vertex
                distTo[vertex]=0;       //dann distance ist 0
            }else if (vertex==s){    // wenn vertex gleich zu start vertex ist
                distTo[vertex] =0;   //  dann distance ist 0
            }else {
                distTo[vertex] = pathTo (vertex).size () -1;
            }
        }
    }
}

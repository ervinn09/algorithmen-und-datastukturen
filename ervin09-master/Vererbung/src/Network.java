...
public class Network {
	...
	public Network(int inter, int receptors, int cortical) {
	    ...
	    /* Photoreceptor photoreceptor = new Photoreceptor (i, receptortypes[i%3]);
		Interneuron interneuron =new Interneuron (i);
		CorticalNeuron corticalNeuron = new CorticalNeuron (i);  */

		if (receptors < 3 || receptors > inter){
			throw new RuntimeException ("weniger als drei Fotorezeptoren initiiert oder weniger Interneuronen als Fotorezeptoren");
		}
		for (int i= 0; i<receptors ; i++) {
			   neurons[i]  = new Photoreceptor (i,receptortypes[i%3]) ;
		   }
		for (int i= 0; i<inter ; i++) {
			neurons[i+receptors]  = new Interneuron (i+receptors) ;
		}
		for (int i= 0; i<cortical ; i++) {
			neurons[i+receptors+inter]  = new CorticalNeuron (i+receptors+inter) ;
		}
	 ...
	 */
		public double[] signalprocessing(double[] input) {
			...
			double[] ooo =countColorreceptors ();
			for (Neuron neuron : this.neurons) {
				if(neuron instanceof Photoreceptor) {
					neuron.integrateSignal (input);
				}
			}
			for (Neuron neuron : this.neurons) {
				if(neuron instanceof CorticalNeuron) {
					double[] n = new double[3];
					n =  ((CorticalNeuron) neuron).getSignal();
					for (int c = 0; c < 3; c++) {
						signal[c] = signal[c] + n[c];
					}
				}
			}
			for (int i = 0; i < 3; i++) {
				signal[i] = signal[i] / ooo[i];
			}
			return signal;
		}

...
public class AStar15Puzzle {
    ...
    public static PartialSolution solveByAStar(Board board) {
        // TODO 1.3 solveAStar
        PartialSolution ps = new PartialSolution (board);
        PriorityQueue<PartialSolution> priorityQueue = new PriorityQueue<PartialSolution> ();
        priorityQueue.add (ps);


        while (!priorityQueue.isEmpty ()){
            PartialSolution partialSolution = new PartialSolution (priorityQueue.poll ());

            if (partialSolution.isSolution ()) {
                return partialSolution;
            }
            for (Move move:partialSolution.validMoves ()) {
                PartialSolution psclone = new PartialSolution (partialSolution);
                psclone.doMove (move);

                if (psclone.isSolution ()) { return psclone; }
                else{ priorityQueue.add (psclone); }
            }
        }
        return null;
    }

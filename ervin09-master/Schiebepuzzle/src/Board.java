...
public class Board {
    ...
    public int manhattan() {
        // TODO 1.1 manhattan
        int result = 0;
        for (int b = 0;b<this.ny;b++){
            for (int a = 0;a<this.nx;a++){
                if (this.field[b][a] != BLANK){
                    result = result + dist (a,b,this.field[b][a]);
                }
            }
        }
        return result;
    }
    public int dist(int xpos , int ypos ,int wert){
        wert = wert -1 ;
        int wertx = wert % this.nx;
        int werty = wert / this.ny;
        int temp1 = Math.abs (wertx - xpos);
        int temp2 =  Math.abs (werty - ypos);
        int sumtemp = temp1+temp2;
        return sumtemp;
    }
    ...
    public static void main(String[] args) {
        String filename = "samples/board-manhattan0.txt";

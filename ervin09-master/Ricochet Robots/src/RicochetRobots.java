...
public class RicochetRobots {
    ...
    public static PartialSolution bfsWithHashing(Board board) {
        /* TODO */
        HashSet<PartialSolution> hashSet = new HashSet<> ();
        PartialSolution partialSolution = new PartialSolution (board);
        hashSet.add (partialSolution);

        Queue<PartialSolution> queue = new LinkedList<> ();
        queue.offer (partialSolution);

        while (!queue.isEmpty ()){
            PartialSolution p1 = new PartialSolution (queue.poll ());
            if (p1.isSol ()){
                return p1;
            }
            for (Move move: p1.validM ()) {
                PartialSolution p2 = new PartialSolution (p1);
                p2.doMove (move);
